...
async def trait(ctx):
    try:
        await ctx.send(pf2bot.get_trait(re.sub(r'!trait ', '', ctx.message.content)))
    except:
...
async def condition(ctx):
    try:
        await ctx.send(pf2bot.get_condition(re.sub(r'!condition ', '', ctx.message.content)))
    except:
...
async def spell(ctx):
    try:
        await ctx.send(pf2bot.get_spell(re.sub(r'!spell ', '', ctx.message.content)))
    except:
